...
function zoom(d,flag,rank) {
  ...
  var transition = d3.transition()
      ...
      .tween("zoom", function(d) {
        ...
        else if(flag=="subleft"){
          i = d3.interpolateZoom(subview_left, [focus.x, focus.y, focus.r * 2 + submargin]);
          return function(t) { zoomTo(i(t),"subleft"); };
        }
        else{
          i = d3.interpolateZoom(subview_right, [focus.x, focus.y, focus.r * 2 + submargin]);
          return function(t) { zoomTo(i(t),"subright"); };
        }
      });
  ...
  if(d.matched!=-1 && rank==1){
      ...
      else if(flag=="subright"){
        subsvgViewer = subsvgViewer_left;
        match_node = svgViewer.ownerDocument.getElementById("subl_"+str_match).__data__;
        flag_pos = "subleft";
      }
      else if(flag=="subleft"){
        svgViewer = svgViewer_right;
        match_node = svgViewer.ownerDocument.getElementById("subr_"+str_match).__data__;
        //match_node = d3.select("#section2").select("#"+str_match);
        flag_pos = "subright";
      }
...
function zoomTo(v,flag) {
  ...
  if(flag=="left"){
    ...
    subnode_left.attr("transform", function(d) { return "translate(" + (d.x - v[0]) * k + "," + (d.y - v[1]) * k + ")"; });
    sublines_left.attr("transform", function(d) {
     return "scale(" + k + ")"+" translate(" + ((d.x1+d.x2)/2*0 - v[0]) * 1 + "," + ((d.y1+d.y2)/2*0 - v[1]) * 1 + ") "; 
    });
    subcircle_left.attr("r", function(d) { return d.r * k; });
  }
  else if(flag=="subright"){
    k = diameter / v[2];
    subview_right = v;

    subnode_right.attr("transform", function(d) { return "translate(" + (d.x - v[0]) * k + "," + (d.y - v[1]) * k + ")"; });
    sublines_right.attr("transform", function(d) {
     return "scale(" + k + ")"+" translate(" + ((d.x1+d.x2)/2*0 - v[0]) * 1 + "," + ((d.y1+d.y2)/2*0 - v[1]) * 1 + ") "; 
    });
    subcircle_right.attr("r", function(d) { return d.r * k; });
  }
  else{
...
// "./data/subyeast.json"  "./yeastnet.json" ./json/ex0419/yeastnet.json
d3.json("./json/ex0419/yeastnet.json", function(error, root) {
...
//"./data/subhuman.json"  "./humannet.json" ./json/ex0419/humannet-bak.json
d3.json("./json/ex0419/humannet-bak.json", function(error, root) {
  if (error) return console.error(error);
  root_tmp = root;
  root = root.nodes[0];
  var focus = root,

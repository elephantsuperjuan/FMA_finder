...
def parse_php_req(p):
    ...
    #print("modul and func:%s" % modul_func)
    ...
    def run(self):
        ...
        try:  
            ...
            if len(firstbuf) < REQUEST_MIN_LEN:               #不够消息最小长度
                #print ("illegal message, length to short: %s" % firstbuf)
                self._socket.close()
                return

            #print ("receive package: %s" % firstbuf)

            firstComma = index(firstbuf, ',')                #查找第一个","分割符
            totalLen = int(firstbuf[0:firstComma])            #消息包总长度
            #print("message length:%d" % totalLen)
            reqMsg = firstbuf[firstComma+1:]
            while (len(reqMsg) < totalLen):    
                reqMsg = reqMsg + self._socket.recv(16 * 1024)

            #调试
            #print ("request package: %s" % reqMsg)

        except Exception as e:  
            #print ('request package exception', e)
            self._socket.close()
            return
